...
cd /opt/RetroFlag
script=rpbian_safeshutdown.py

if [ -e $script ];
	then
		echo "Script SafeShutdown.py already exists. Doing nothing."
	else
		wget "https://raw.githubusercontent.com/DavidOliM/retroflag-picase/Raspbian/rpbian_safeshutdown.py"
fi
#-----------------------------------------------------------
...
if grep -q "sudo python3 \/opt\/RetroFlag\/rpbian_safeshutdown.py \&" "$RC";
	then
		echo "File /etc/rc.local already configured. Doing nothing."
	else
		sed -i -e "s/^exit 0/sudo python3 \/opt\/RetroFlag\/rpbian_safeshutdown.py \&\n&/g" "$RC"
